  ...
  if (format === 'currency') {
    ...
    if (symbolIndex !== -1) {
      ...
      if (separatorIndex !== -1) {
        ...
        result.decimalPart =
          decimalSeparator + valueWithoutSymbol.substring(separatorIndex + 1);
      } else {
    ...
    if (percentIndex !== -1) {
      ...
      const valueWithoutPercent = formattedValue
      ...
      if (separatorIndex !== -1) {
        ...
        result.decimalPart =
          decimalSeparator + valueWithoutPercent.substring(separatorIndex + 1);
      } else {
    ...
    if (separatorIndex !== -1) {
      ...
      result.decimalPart =
        decimalSeparator + mainPart.substring(separatorIndex + 1);
    } else {
      result.integerPart = mainPart;
    }
  }
...
export const formatNumber = (
  ...
  useShortFormat: number | boolean = false,
  ...
  const applyShortFormat = (num: number): string => {
    // Define thresholds and corresponding suffixes, including new higher-order magnitudes
    const thresholds = [
      { value: 1e15, suffix: 'PB' },
    ...
    // Determine max digits to show based on useShortFormat parameter
    const maxDigits = typeof useShortFormat === 'number' ? useShortFormat : 3;

    // Calculate decimal precision dynamically
    const calculatePrecision = (normalizedNum: number) => {
      // If decimalPlaces is explicitly set, use that
      if (decimalPlaces !== undefined) return decimalPlaces;

      // Otherwise calculate precision based on maxDigits
      const integerDigits = Math.floor(normalizedNum).toString().length;
      return Math.max(0, maxDigits - integerDigits);
    };

    // Find the appropriate threshold
    for (const { value, suffix } of thresholds) {
      if (num >= value) {
        const normalizedNum = num / value;
        const precision = calculatePrecision(normalizedNum);
        return normalizedNum.toFixed(precision) + suffix;
      }
    }

    // For small numbers, apply regular formatting
  ...
  const applyCurrencyShortFormat = (num: number): string => {
    ...
    for (const { value, suffix } of thresholds) {
      if (num >= value) {
        const normalizedNum = num / value;
        const precision = calculatePrecision(normalizedNum);
        shortNumStr = normalizedNum.toFixed(precision) + suffix;
        break;
      }
    }
    ...
    // For small numbers, apply regular formatting
    if (!shortNumStr) {
      shortNumStr = num.toFixed(decimalPlaces);
    }

    // 根据货币符号位置决定返回格式
    const currencyPosition = parts.findIndex(
      (part) => part.type === 'currency',
    );
    const literalAfterCurrency =
      parts.find(
        (part, index) => index > currencyPosition && part.type === 'literal',
      )?.value || '';
